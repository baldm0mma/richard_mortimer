...
export class Card extends Component {
  ...
  handleClick = () => {
    this.props.onChoosingCriminal(this.props.character.id, this.props.character.name);
  };

  render = () => {
    return (
      <Link to={`/criminals/${this.props.character.name}`} className='link_to_single'>
        <article className='allCharacter_card grow' onClick={this.handleClick}>
          <h2>{this.props.character.name}</h2>
          <img
            className={
              this.props.character.status === 'Dead' ? 'grid_image grey' : 'grid_image'
            }
